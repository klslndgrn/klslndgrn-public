...
# The function below, `testClassifier`, will be used to try out the different
# datasets. `fetchDataset` can be provided with any of the dataset arguments
# `wine`, `iris`, `olivetti` and `vowel`.
    ...
    for trial in range(ntrials):
        ...
        if trial % 10 == 0:
            print(
                ...
                trial,
                "Accuracy",
                "%.3g" % (100 * np.mean((yPr == yTe).astype(float))),
            )
...
def plotBoundary(classifier, dataset="iris", split=0.7):
    ...
    # colormap = colormap.reshape(1, -1)

    fig = plt.figure()
    # plt.hold(True)
    conv = ColorConverter()
    for (clr, c) in zip(colormap, classes):
        try:
            CS = plt.contour(
                xRange,
                yRange,
...
class DecisionTreeClassifier(object):
    ...
    def trainClassifier(self, Xtr, yTr, W=None):
        ...
        rtn.classifier = tree.DecisionTreeClassifier(
            max_depth=int(Xtr.shape[1] / 2 + 1)
        )
        if W is None:
